...
export default class Select extends mixin(React.Component).with(
  Scrim,
  Transition
) {
  ...
  render() {
    ...
    const {
      ...
      onExited,
      ...props
    } = mergeProps(this.props, { className: ['select', { open }] });
    const toggleValue = isBlank(controlledValue)
      ? uncontrolledValue
      : controlledValue;

    const { toggleLabel, selectOptions } = options.reduce(
      (memo, option) => {
        const { value, label } =
    ...
    return (
      <div {...props}>
        ...
        >
          ...
          <Iconography src="select_chevrons" />
        </UIButton>
        {list}
