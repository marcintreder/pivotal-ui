...
export class AutocompleteList extends React.Component {
  static propTypes = {
    ...
    children(props, name) {
      if (props[name] && props[name].length)
        return new Error('AutocompleteList can only wrap one element');
    },
  ...
  renderSuggestionList() {
    ...
    const suggestions = suggestedValues.map((suggestion, key) => {
      ...
      const className = classnames(
        ...
        {
          highlighted:
            key === this.props.$autocomplete.get('highlightedSuggestion')
        },
    ...
    if (!suggestions.length) {
      const result = showNoSearchResults ? (
        <div>
          <ul>
            <li className="autocomplete-list autocomplete-item autocomplete-item-no-results">
              No search results
            </li>
          </ul>
        </div>
    ...
    return (
      <ul className={classnames('autocomplete-list', className)}>
        {suggestions}
      </ul>
    );
  }
  ...
  render() {
    ...
    children = React.Children.map(children, e =>
