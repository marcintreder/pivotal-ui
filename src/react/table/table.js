...
export class Table extends TablePlugin {
  ...
  render() {
    ...
    const renderedColumns = (columns || dataColumns || []).map(
      attribute => (typeof attribute === 'string' ? { attribute } : attribute)
    );
    ...
    const headers = renderedColumns.map((column, key) => {
      ...
      return (
        <Th {...{ key, ...this.plugProps('th', { children }, thContext) }} />
      );
    });
    ...
    const headerRow = (
      <HeaderTr
        {...this.plugProps('tr', { children: headers }, headerTrContext)}
      />
    );

    const bodyCols = rowDatum =>
      renderedColumns.map((column, key) => {
        ...
        return (
          <Td {...{ key, ...this.plugProps('td', { children }, tdContext) }} />
        );
      });
    ...
    const bodyRows = data.map((rowDatum, key) => {
      ...
      return (
        <Tr
          {...{
            key,
            ...this.plugProps('tr', { children: bodyCols(rowDatum) }, trContext)
          }}
        />
      );
    });
