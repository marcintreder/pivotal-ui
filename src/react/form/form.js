...
const getFieldEntries = fields =>
  Object.entries(fields).filter(([name, props]) => props);
const isOptional = ({ optional }, current) =>
  typeof optional === 'function' ? optional({ current }) : optional;
const isPromise = promise => promise && typeof promise.then === 'function';
const newId = () => crypto.randomBytes(16).toString('base64');
const noop = () => undefined;

const newFormState = (fields, ids, cb) =>
  getFieldEntries(fields).reduce(
    (memo, [name, props]) => {
...
const newInitialValue = initialValue =>
  [null, undefined].includes(initialValue) ? '' : initialValue;
...
export default class Form extends React.Component {
  ...
  shouldComponentUpdate({ fields }, nextState) {
    ...
    const { initial: newInitial, ids, current: newCurrent } = newFormState(
      fields,
      nextState.ids,
      ({ name, initialValue }) => {
        initialValue = newInitialValue(initialValue);
        return {
          ...
          currentValue: current.hasOwnProperty(name)
            ? current[name] === this.state.initial[name]
  ...
  canReset = () =>
    ...
    !deepEqual(this.state.initial, this.state.current);
  ...
  canSubmit = ({ checkRequiredFields } = {}) => {
    ...
    return (
      ...
      find(
        Object.keys(initial),
        key => !deepEqual(initial[key], current[key])
      ) &&
      (checkRequiredFields
        ? checkRequiredFields(this.state.current)
        : !find(
            Object.keys(fields).filter(
              name => fields[name] && !isOptional(fields[name], current)
            ),
            name => !current[name]
          )) &&
      !find(
        getFieldEntries(fields),
        ([name, { validator }]) =>
          validator && validator(this.state.current[name])
      )
    );
  };
  ...
  onSubmit = e => {
    ...
    const onSuccess = response => {
      ...
      return isPromise(onModifiedPromise)
        ? onModifiedPromise.then(after)
        : after();
    };
  ...
  setValues = (values, cb) =>
    this.setState({ current: { ...this.state.current, ...values } }, cb);
