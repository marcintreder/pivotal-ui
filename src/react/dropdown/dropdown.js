...
const defaultToggleNode = (
  ...
  return (
    ...
    >
      {title}
      {showIcon && <Iconography src={icon} className="icon-toggle" />}
    </button>
  );
};

export default class Dropdown extends mixin(React.Component).with(
  Scrim,
  Transition
) {
  ...
  render() {
    ...
    const dropdownMenuClasses = classnames('dropdown-menu', {
      ...
      'dropdown-menu-float':
        split || flat || link || floatMenu || noTitle || menuAlign !== 'none',
    ...
    const dropdownOptions = (
      <div className={dropdownMenuClasses}>
        <ul aria-label="submenu" onClick={this.menuClick}>
          {React.Children.map(
            children,
            child => (child ? <li className={itemClassName}>{child}</li> : null)
          )}
        </ul>
      </div>
    );
    ...
    return (
      <div className={dropdownClasses} {...props}>
        ...
        {blockingScrim &&
          open &&
          !disableScrim && <div className="scrim" onClick={this.scrimClick} />}
        {dropdownOptions}
      </div>
    );
  }
